...
app.config['MAIL_PASSWORD'] = 'julz upes onmw xwrs '
...
def register():
    ...
    if request.method == 'POST':
        ...
        if (len(password) < 8 or not re.search(r'[A-Z]', password)
                or not re.search(r'[a-z]', password) or not re.search(
...
def profile():
    ...
    and fetches non-sensitive user data from the database. It then constructs the profile picture URL,
    fetches counts of followers and following, checks if the logged-in user is following the viewed user,
    and renders the appropriate profile template.
...
# Define your custom time function
def your_custom_time_function(created_at):
    # Calculate the time difference in days
    days_difference = (datetime.utcnow() - created_at).days
...
def your_posts():
    if 'user_id' in session:
        ...
        return render_template('your_posts.html', user_posts=user_posts, total_posts=total_posts,
                               posts_per_page=posts_per_page, page=page,  your_custom_time_function=your_custom_time_function)
    else:
        flash('You need to log in first.', 'error')
        return redirect(url_for('login'))
...
def view_posts():
    ...
    if category == 'title':
        ...
        sql_condition = ('LOWER(posts.title) LIKE LOWER(%s) OR LOWER(posts.content) LIKE LOWER(%s)'
...
def full_post(post_id):
    ...
    if post_owner_id:
        ...
        return render_template('full_post.html', title=title, content=content_escaped,
                               username=username, created_at=created_at, edited_at=edited_at,
                               is_edited=is_edited, profile_picture=profile_picture, post_id=post_id,
                               user_id=user_id, post_owner_id=post_owner_id[0], is_following=is_following,
