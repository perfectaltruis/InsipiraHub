...
def register():
    ...
    if request.method == 'POST':
        ...
        if len(first_name) < MIN_LENGTH or len(last_name) < MIN_LENGTH or len(username) < MIN_LENGTH:
            flash('Name and username must be at least {} characters long.'.format(
                MIN_LENGTH), 'error')
            return redirect(url_for('register'))
    ...
    try:
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute("SELECT * FROM admins WHERE email = %s AND username = %s AND password = %s",
...
def create_user():
    ...
    if request.method == 'POST':
        ...
        # Insert the new user into the database
        cursor = conn.cursor()
        cursor.execute("INSERT INTO accounts (email, first_name, last_name, username, password, country, "
                       "registration_date, day, month, year, security_pin, user_verified, role) "
                       "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 't', 'USER')",
                       (email, first_name, last_name, username, hashed_password, country,
                        registration_date, day, month, year, security_pin))
        conn.commit()
        cursor.close()
    ...
    if 'role' in session and session['role'] == 'admin':
        if request.method == 'POST':
            ...
            try:
                ...
                if sql_statement in ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'ALTER', 'TRUNCATE', 'CREATE TABLE',
                                     'DROP TABLE', 'CREATE INDEX', 'DROP INDEX', 'GRANT', 'REVOKE', 'BEGIN', 'COMMIT',
                                     'ROLLBACK', 'CREATE DATABASE', 'DROP DATABASE', 'CREATE SCHEMA', 'DROP SCHEMA',
                                     'CREATE FUNCTION', 'DROP FUNCTION', 'CREATE TRIGGER', 'DROP TRIGGER',
                                     'CREATE VIEW', 'DROP VIEW', 'CREATE SEQUENCE', 'DROP SEQUENCE']:
                    ...
                    return render_template('admin_custom_query.html', result=result, query=sql_query)
                ...
                if isinstance(e, ProgrammingError):
                    if "relation" in str(e) and "does not exist" in str(e):
                        flash(
                            'Table does not exist. Please check your SQL query.', 'error')
                        print('Table does not exist.')
                    elif "column" in str(e) and "does not exist" in str(e):
                        flash(
                            'Column does not exist. Please check your SQL query.', 'error')
...
def login():
    """
        ...
        If the request method is POST, retrieve the entered username and password.
        Check if the username exists in the database and verify the entered password.
        If the user is verified and has 2FA enabled, generate a token, send it via email, and proceed to 2FA verification.
        If 2FA is not enabled, proceed to the dashboard and store user data in the session.
        If the user is not verified, render an account not verified template.
        If the username or password is incorrect, render a login error template.
        ...
        if user and check_password_hash(user[5], password):
            ...
            if user[12]:  # user_verified column is the 13th column in the table accounts
                ...
                if user[14] == 'T':  # 2fa_status column is the 15th column in the table accounts
                    ...
                    send_2fa_token_email(user[1], token,
                    ...
                    print(
                        f"Setting session variables: user_id={user[0]}, username={user[4]}, email={user[1]}, first_name={user[2]}, last_name={user[3]}, 2fa_token={token}")
                    ...
                    # Before redirecting or rendering templates, print the session variables again to confirm their values
                    ...
                    session['user_id'] = user[0]
                    print(
                        f"Setting session variables: user_id={user[0]}, username={user[4]}, email={user[1]}, first_name={user[2]}, last_name={user[3]}")
                    print(
...
def verify_2fa():
    ...
    if stored_token and entered_token == stored_token:
        ...
        token_expiration_time = token_timestamp + \
            timedelta(minutes=10)  # Set your desired expiration time
...
def verify_email(token):
    ...
    This route is responsible for processing the verification token provided in the URL.
...
def dashboard():
    ...
    This route checks if a user is logged in, fetches their user data, including username and profile picture,
    from the database. It then retrieves counts of followers and following, prints the logged-in user's username,
    fetches usernames and profile URLs of followers, and renders the dashboard template.
    ...
    if 'user_id' in session:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT username, profile_picture FROM accounts WHERE id = %s", (user_id,))
        user = cursor.fetchone()
        ...
        if user:
            ...
            return render_template('dashboard.html', username=username,
                                   ...
                                   following_count=following_count, user=user, user_id=user_id)
        else:
...
def profile():
    ...
    This route checks if a user is logged in, retrieves the user_id from the query parameters, and fetches non-sensitive user data
    from the database. It then constructs the profile picture URL, fetches counts of followers and following, checks if the logged-in
    user is following the viewed user, and renders the appropriate profile template.
...
def edit_profile():
    ...
    This route allows logged-in users to edit their profile information. If the request method is POST, it updates the user's
    first name and last name based on the form submission. It then commits the changes to the database and redirects the user
    to the account page with a success message. If the request method is GET, it fetches the user's data from the database
    and renders the edit profile form with pre-filled data.
...
def resend_verification_email():
    """
        ...
        This route allows users to request a new verification email in case the initial email was not received or expired.
        If the provided email exists in the database and is associated with a non-verified account, a new verification token
        is generated, stored in the table tokens, and sent to the user via email for verification. If the email is already
...
def resend_verification():
    """
        Handle the functionality of resending email verification to users.

        This route allows users to request a resend of the email verification link. If the user exists and has not
        been verified yet, a new verification token is generated, and the verification link is sent to the user's email
        address. The user is notified through a confirmation page. If the user is already verified, they are informed
        that the email has already been verified. If the provided email does not match any user, an error page is displayed.

        Returns:
            str: A rendered template based on the outcome of the verification process.
        """
    if request.method == 'POST':
        email = request.form['email']
        cursor = conn.cursor()
    ...
    if 'user_id' in session:
        if request.method == 'POST':
            ...
            if request.form['old_email'] != old_email:
                flash(
                    'The provided current email does not match the email associated with your account.', 'error')
    ...
    if token_data:
        ...
        if time_difference > 30:
            ...
            try:
                ...
                flash(
                    'Database error occurred. Please try again later or contact support.', 'error')
                return redirect(url_for('update_email'))
                ...
                flash(
                    'An unexpected error occurred. Please try again later or contact support.', 'error')
                return redirect(url_for('update_email'))
...
def send_activation_email(email, activation_status, username):
    """
       Sends an email notification to the user confirming the activation or deactivation of Two-Factor Authentication (2FA).
...
def delete_account():
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute('SELECT "2fa_status", email, username, password, security_pin FROM accounts WHERE id = %s',
                           (user_id,))
            user_data = cursor.fetchone()
    ...
    if current_2fa_status == 'T' and request.method == 'POST':
        ...
        if (entered_email == stored_email and
                entered_username == stored_username and
                is_valid_password(entered_email, entered_password) and
                entered_security_pin == stored_security_pin):
            ...
            send_2fa_token_email(stored_email, f'Your 2FA token for account deletion is: {two_fa_token}',
                                 stored_username)

            # Store the token in the session for verification
            session['verification_token'] = two_fa_token
            flash(
                'A 2FA token has been sent to your email address. Please check your email to confirm account deletion.',
                'info')
            return render_template('2fa_deletion_verification.html')

        else:
            flash(
                'Invalid email, username, password, or security pin. Please try again.', 'error')
        ...
        if (entered_email == stored_email and
            ...
            with get_db_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute(
                        'INSERT INTO deleted_accounts (email, first_name, last_name, country, day, month, year, deleted_date, deletion_reason) '
                        'SELECT email, first_name, last_name, country, day, month, year, %s, %s FROM accounts WHERE id = %s',
...
def send_2fa_deletion_token_email(email, token):
    ...
    """
    print(
        f"Sending token: {token} to email: {email}")  # Print the token and email for debugging
...
def send_account_deletion_confirmation_email(email, username):
    """
        ...
        This function constructs and sends an email confirming the successful deletion of the user's account.
        The email includes a personalized greeting using the user's username.
    ...
    message_body = (
        f"Hello {username},\n\n"
        f"We wanted to let you know that your account has been successfully deleted. If you did not initiate this action "
        f"or have any concerns, please don't hesitate to reach out to our support team immediately.\n\n"
        f"We appreciate your time with us and thank you for being a part of our community. If you ever decide to come "
        f"back, we'll be here to welcome you!\n\n"
        f"Best regards,\n"
        f"The Intuitivers Team"
    )
...
def verify_2fa_deletion():
    """
       ...
       If the tokens match, the user's account is deleted, a confirmation email is sent, and the user is redirected to
       the account_deleted_success.html template. If the tokens do not match, an error message is displayed,
       and the user is redirected to the delete_account route.
...
def forbidden(error):
    return render_template('ErrorHandler/ClientError/Forbidden.html'), 403
...
def precondition_failed(error):
    return render_template('ErrorHandler/ClientError/PreconditionFailedError_412.html'), 412


# Custom error handler for 415 Unsupported Media Type Error
@app.errorhandler(415)
def unsupported_media_type(error):
    return render_template('ErrorHandler/ClientError/UnsupportedMediaTypeError_415.html'), 415
...
def unavailable_for_legal_reasons(error):
    return render_template('ErrorHandler/ClientError/UnavailableForLegalReasonsError_451.html'), 451


# Custom error handler for 413 Request Entity Too Large error
@app.errorhandler(413)
def request_entity_too_large(error):
    return render_template('ErrorHandler/ClientError/RequestEntityTooLargeError_413.html'), 413
...
def service_unavailable(error):
    return render_template('ErrorHandler/ServerError/ServiceUnavailableError_503.html'), 503
...
def handle_exception(error):
    ...
    return render_template('ErrorHandler/GenericError.html'), 500
