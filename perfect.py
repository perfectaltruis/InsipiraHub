...
def register():
    if request.method == 'POST':
        ...
        congratulatory_message.body = (
            ...
            f'Best regards,\n'
            f'The Perfect Social Media Community Team'
        )
...
def authenticate_admin(email, username, password):
    try:
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                # Check if the provided username, email, and password match an admin account
    ...
    if 'role' in session and session['role'] == 'admin':
        if request.method == 'POST':
            ...
            try:
                ...
                if sql_statement in ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'ALTER', 'TRUNCATE', 'CREATE TABLE', 'DROP TABLE', 'CREATE INDEX', 'DROP INDEX', 'GRANT', 'REVOKE', 'BEGIN', 'COMMIT', 'ROLLBACK', 'CREATE DATABASE', 'DROP DATABASE', 'CREATE SCHEMA', 'DROP SCHEMA', 'CREATE FUNCTION', 'DROP FUNCTION', 'CREATE TRIGGER', 'DROP TRIGGER', 'CREATE VIEW', 'DROP VIEW', 'CREATE SEQUENCE', 'DROP SEQUENCE']:
        ...
        if user and check_password_hash(user[5], password):
            ...
            if user[12]:  # user_verified column is the 13th column in the table accounts
                ...
                if user[14] == 'T':  # 2fa_status column is the 15th column in the table accounts
                    ...
                    # user[1] is the email address (2nd column in the table accounts)
                    send_2fa_token_email(user[1], token, username)

                    # Insert the token into the accounts table
...
def send_deleted_account_email(email):
    ...
    body = ("We want to inform you that your account has been successfully deleted. "
            ...
            f"Thank you for being with us. If you need further assistance, please don't hesitate to contact our support team.")
    ...
    try:
        ...
        flash('Account deletion email sent successfully.', 'success')
        return True
    except smtplib.SMTPAuthenticationError:
        print('Failed to send account deletion email: Invalid credentials or account access disabled.')
        flash('Failed to send account deletion email: Invalid credentials or account access disabled.', 'error')
        return False
    except smtplib.SMTPConnectError:
        print('Failed to send account deletion email: Could not establish a connection with the email server.')
        flash('Failed to send account deletion email: Could not establish a connection with the email server.', 'error')
        return False
    except smtplib.SMTPException as e:
        print(f'Failed to send account deletion email: {e}')
        flash(f'Failed to send account deletion email: {e}', 'error')
        return False
    except Exception as e:
        print(f'Failed to send account deletion email: {e}')
        flash(f'Failed to send account deletion email: {e}', 'error')
...
def send_activation_email(email):
    ...
    msg.body = ('You requested to activate 2FA in your account and we did it, Now your account is protected with'
                ' additional security layer of protection.  Yo receive this email to inform you that your account has '
...
def page_not_found(error):
    return render_template('NotFoundError_404.html'), 404


# Custom error handler for 500 Internal Server Error
@app.errorhandler(500)
def internal_server_error(error):
    return render_template('InternalServerError_500.html'), 500


# Custom error handler for 413 Request Entity Too Large error
@app.errorhandler(413)
def request_entity_too_large(error):
    return render_template('RequestEntityTooLargeError_413.html'), 413
